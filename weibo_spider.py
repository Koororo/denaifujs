...
#设置heades
headers = {
    'Cookie': 'BAIDU_SSP_lcr=https://login.sina.com.cn/; WEIBOCN_FROM=1110005030; SUB=_2A25JBvmfDeRhGeFG6loW8yfJyz6IHXVqCIfXrDV6PUJbkdAGLVOtkW1NfjQWMYy11c6ZKnsgrgpeMwUnWUy7JgeS; _T_WM=12193563165; MLOGIN=1; XSRF-TOKEN=d8fa33; mweibo_short_token=88b4436c03; M_WEIBOCN_PARAMS=oid%3D4875298702888816%26luicode%3D20000061%26lfid%3D4875298702888816%26uicode%3D20000061%26fid%3D4875298702888816',
...
comments_ID = []
def get_title_id():
    for page in range(1,54):  #每个页面大约有54个话题
        headers = {
            "User-Agent" : UserAgent().chrome #chrome浏览器随机代理
        }
        time.sleep(3)
        #该链接通过抓包获得
        api_url = r'https://m.weibo.cn/api/container/getIndex?containerid=100103type%3D1%26q%3D%E9%AB%98%E5%BD%A6&page_type=searchall&page=' + str(page)
        print(api_url)
        rep = requests.get(url=api_url, headers=headers, verify=False)
        #获取ID值并写入列表comment_ID中
        for i in re.findall('"mid":"(\d+)",', rep.text):
...
def spider_title(comment_ID):
    try:
        ...
        html_text = requests.get(url=article_url, headers=headers, verify=False).text
        time.sleep(3)
        #话题内容
        ...
        #日期
        title_created_YMD = "{}/{}/{}".format(created_title_time[-1], month.index(created_title_time[1])+1, created_title_time[2])
...
def write_csv(jsondata):
    for json in jsondata['data']['data']:
        ...
        created_YMD = "{}/{}/{}".format(created_times[-1], month.index(created_times[1])+1, created_times[2])
